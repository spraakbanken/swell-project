...
In terms of workflow, a key assumption in our approach is that it is beneficial to separate error coding into two steps: normalisation (that is, correction of the learner text according to the target hypotheses) and the actual annotation of the corrections. There are several reasons for this division: First, by letting the annotator work with the text prior to the error annotation, they will be in a better position to build a coherent understanding of the learner's linguistic competence and to form target hypotheses that are consistent with this understanding. Secondly, since error correction and error annotation are conceptually distinct, it is logical to perform them as separate tasks. Thirdly, normalisation can be outsourced to researchers in second-language acquisition, that is, people who are experts in comparing target hypotheses for learner texts. Finally, a prerequisite for calculating inter-annotator agreement for error annotation is that this has used the same normalised text.

The clear separation between the learner text and the normalised text allows our system (Svala) to automatically construct a parallel corpus with alignments at the word level. Because of this, normalisation actually involves two subtasks: corrections of the learner text to obtain its normalisation, and changes of the word alignments to the extent that the automatically generated links are incorrect. An additional, independent advantage of the approach is that the resulting parallel corpus will be a highly valuable resource for development of automatic methods for identifying learner errors.
