...
Normalization in SVALA means editing of the original learner text in such a way that the sentences a) are correct with respect to orthography, morphology and syntax and b) are semantically coherent. The first point corresponds to the Minimal Target Hypothesis in the German learner corpus Falko ([Reznicek et al. 2012, page 42 ff.](https://www.linguistik.hu-berlin.de/de/institut/professuren/korpuslinguistik/forschung/falko)), whereas the latter point goes further than the Minimal Target Hypothesis by allowing changes that affect meaning. A German example that highlights the difference (from [Boyd 2018, slide 7](https://sweclarin.se/sites/sweclarin.se/files/event_atachements/L2wsh_Boyd_slides.pdf)) is the Minimal Target Hypothesis "Rufst du deine Antwort an?" ("Do you call your answer?"), which is semantically anomalous. To represent correction of semantics, pragmatics and stylistics, there is an additional level in Falko called the Extended Target Hypothesis. A corresponding such hypothesis which is semantically coherent is "Rufst du mich wegen deiner Antwort an?". The reason that the target hypothesis in SVALA goes beyond the Minimal Target Hypothesis in Falko is that SVALA only has a single level of target hypotheses. 
