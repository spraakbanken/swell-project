...
As for workflow, a key assumption in our approach is that it is beneficial to separate error coding into two steps: normalisation (that is, correction of the learner text according to the target hypotheses) and the actual annotation of the corrections. There are several reasons for this division: First, by letting the annotator work with the text prior to the error annotation, they will be in a better position to build a coherent understanding of the learner's linguistic competence and to form target hypotheses that are consistent with this understanding. Secondly, since error correction and error annotation are conceptually distinct tasks, their quality can improve if they are also carried out as separate tasks. Thirdly, normalisation can be outsourced to researchers in second-language acquisition, that is, people who are experts in learner texts and in reasoning about target hypotheses. Finally, being able to compare different error annotations for the same normalisation is a prerequisite for calculating inter-annotator agreement.

System-wise, the clear separation between the learner text and the normalised text allows our system (Svala) to automatically construct a parallel corpus, with alignments on the word level. Because of this, normalisation actually involves two subtasks: corrections of the learner text to obtain its normalised version, and change of the word alignments to the extent that the automatically generated ones need correction. An additional, independent advantage is that the resulting parallel corpus will be a highly valuable resource for development of automatic methods for identifying learner errors.
