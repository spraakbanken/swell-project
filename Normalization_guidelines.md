...
A key assumption of our approach is that it is beneficial to separate error coding into two steps: normalisation (that is, correction of the learner text according to the target hypotheses) and the actual annotation of the corrections. There are several reasons for this division: First, by letting the annotator work with the text prior to the actual error annotation, they will be in a better position to build an understanding of the learner's linguistic competence and to form coherent target hypotheses. Secondly, since error correction and error annotation are conceptually distinct tasks, their quality can improve if they are indeed performed as separate tasks. Thirdly, normalisation can be outsourced to researchers in second-language acquisition, that is, people who are experts in learner texts. Finally, it guarantees that annotators have worked off the same normalised texts, which is a prerequisite for calculating inter-annotator agreement.
