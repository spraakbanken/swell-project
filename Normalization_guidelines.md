...
Normalisation is not an unambiguous concept, but the aim of this guideline is to make the notion as precise as possible. Still, there will typically be several possible normalisations of a text, just as there are several correct translations of a sentence. Why do we then do normalisation? A key purpose is to make the target hypothesis explicit, given that target hypotheses always exist regardless of whether we make them explicit or not. (Note, by the way, that our tool only allows for a single target hypothesis.) Another purpose is that we expect the resulting parallel text to be a very valuable resource for development of automatic methods for identifying learner errors. Notably, this resource will be valuable even if there are several ways in which the text could have been normalised. A third purpose is more speculative, but our intuition is that manual error coding will become more efficient if our tool clearly separates what are conceptually different tasks (error detection, correction and annotation).
